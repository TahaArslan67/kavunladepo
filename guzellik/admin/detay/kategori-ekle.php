...
?>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-10">
                                                    <select class="form-control" name="kategori">
                                                        ...
                                                        <?php
                                                        $cek = $db->query("select * from urun_kategori where durum='0' and not id='{$guncelle["id"]}' order by id desc")->fetchAll(PDO::FETCH_ASSOC);
                                                        foreach($cek as $goster){
                                                        ?>
                                                        <option value="<?=$goster["id"]?>" <?php if($guncelle["durum"]==$goster["id"]){ echo ' selected';}?>><?=$goster["adi"]?></option>
                        ...
                        <div class="row" id="resimler">
                            	...
                            	<?php
									...
									$iddd=$_GET['id'];
									if($_GET['islem']=='duzenle'){
										$cek = $db->query("SELECT * FROM urun_img WHERE urun_id = '$iddd' and tur='urun_kategori' order by id asc", PDO::FETCH_ASSOC);
										if($cek->rowCount()){
											foreach( $cek as $c ){
...
<div id="queue"></div>
                                             ...
                                             <div class="form-group ">
                                                <div class="col-sm-offset-11 col-sm-1">
                                                    <?php if($_GET["islem"]=='duzenle'){?>
                                                    <input type="hidden" name="link" value="../kategori-ekle?islem=duzenle&id=<?=$guncelle["id"]?>">
                                                    <input type="hidden" name="id" value="<?=$guncelle["id"]?>">
                                                  <button type="submit" name="kategori-guncelle" class="btn btn-info waves-effect waves-light">Güncelle</button>
                                                  <?php } else {?>
                                                  <input type="hidden" name="link" value="../kategori-ekle">
                                                  <button type="submit" name="kategori-ekle" class="btn btn-info waves-effect waves-light">Ekle</button>
                                                  <?php }?>
                                                </div>
                                            </div>
